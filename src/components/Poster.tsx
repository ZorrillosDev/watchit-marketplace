                        ...
                        avatar={
                            <AvatarGroup max={2}>
                                <Tooltip title={`Owner: ${props.owner.username}`}>
                                    <Avatar alt={`${props.owner.username} profile image`}
                                            src={`${props.owner.profileUrl}`} sx={{width: 24, height: 24}}/>
                                </Tooltip>
                                <Tooltip title={`Creator: ${props.creator.username}`}>
                                    <Avatar alt={`${props.creator.username} profile image`}
...
export const PosterWrapper = styled(Card, {
    ...
    border: showDetails ? '1px solid #eee' : '',
    boxShadow: 'none',
    cursor: 'pointer',
    width: '100%',
    height: '100%',
...
export const PosterMediaContent = styled(CardContent, {
    ...
    img: {
        ...
        maxWidth: '100%',
        borderRadius: '6px',
        width: showDetails ? '65% !important' : '100%'
    }
}))
